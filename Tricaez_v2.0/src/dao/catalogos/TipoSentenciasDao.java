 ...
 * @author mavg
 */
public class TipoSentenciasDao extends ConexionMySql{

    Utilidades u = new Utilidades();
    
    public TipoSentenciasDao() {
    }
    ...
    public ArrayList getTipoSentencias(){
        ...
        ResultSet rs = null;
        String query = "SELECT id_tipo_sentencia, nombre, observaciones, seleccionar FROM cat_tipo_sentencias ORDER BY nombre";
        try{
            rs = con.createStatement().executeQuery(query);
            while (rs.next()){
                array.add(new TipoSentenciaEntity(rs.getString(1),rs.getString(2),rs.getString(3),rs.getBoolean(4)));
            }
    ...
    public ErrorEntity saveTipoSentencia(Connection conx, TipoSentenciaEntity tipo, SesionEntity sesion){
        ErrorEntity error = new ErrorEntity(true);
        try{
            String query = "INSERT INTO cat_tipo_sentencias VALUES(?,?,?,?,?,?,?)";
            PreparedStatement save = conx.prepareStatement(query);
            save.setString(1, tipo.getId_tipo_sentencia());
    ...
    public ErrorEntity updateTipoSentencia(Connection con, TipoSentenciaEntity tipo, SesionEntity sesion){
        ErrorEntity error = new ErrorEntity(true);
        try{
            String query = "UPDATE cat_tipo_sentencias SET nombre = ?, observaciones = ?, seleccionar = ?, "
                    + "id_usuario = ?, f_cambio = ? WHERE id_tipo_sentencia = ?";
            ...
            save.setString(6, tipo.getId_tipo_sentencia());
            error.setError(save.execute());
        }catch (SQLException e){
    ...
    public ErrorEntity deleteTipoSentencia(Connection con, String id){
        ErrorEntity error = new ErrorEntity(true);
        String query = "DELETE FROM cat_tipo_sentencias WHERE id_tipo_sentencia = '"+id+"'";
        try{
    ...
    public String getId(){
        ...
        Connection con = getConexionBD();
        ResultSet rs = null;
        String query = "SELECT IFNULL(MAX(CAST(SUBSTRING(id_tipo_sentencia,2) as SIGNED INTEGER)),0) FROM cat_tipo_sentencias";
        try{
            Integer valor = 0;
            rs = con.createStatement().executeQuery(query);
            while (rs.next()){
    ...
    public ErrorEntity updateSeleccionar(Connection con){
        ErrorEntity error = new ErrorEntity(true);
        try{
            error.setError(con.prepareStatement("UPDATE cat_tipo_sentencias SET seleccionar = false").execute());
        }catch (SQLException e){
            error.setTipo(new ErroresClase(e,"Error al modificar los datos").getErrorSql());
        }catch (Exception ex){
            error.setTipo(new ErroresClase(ex,"Error al modificar los datos").getErrorJava());
