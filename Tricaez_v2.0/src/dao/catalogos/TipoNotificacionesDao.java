 ...
 * @author mavg
 */
public class TipoNotificacionesDao extends ConexionMySql{

    Utilidades u = new Utilidades();
    
    public TipoNotificacionesDao() {
    }
    ...
    public ArrayList getTipoNotificaciones(){
        ...
        ResultSet rs = null;
        String query = "SELECT id_tipo_notificacion, nombre, observaciones, seleccionar FROM cat_tipo_notificaciones";
        try{
            rs = con.createStatement().executeQuery(query);
            while (rs.next()){
                array.add(new TipoNotificacionEntity(rs.getString(1),rs.getString(2),rs.getString(3),rs.getBoolean(4)));
            }
    ...
    public ErrorEntity saveTipoNotificacion(Connection conx, TipoNotificacionEntity tipo, SesionEntity sesion){
        ErrorEntity error = new ErrorEntity(true);
        try{
            String query = "INSERT INTO cat_tipo_notificaciones VALUES(?,?,?,?,?,?,?)";
            PreparedStatement save = conx.prepareStatement(query);
            save.setString(1, tipo.getId_tipo_notificacion());
    ...
    public ErrorEntity updateTipoNotificacion(Connection con, TipoNotificacionEntity tipo, SesionEntity sesion){
        ErrorEntity error = new ErrorEntity(true);
        try{
            String query = "UPDATE cat_tipo_notificaciones SET nombre = ?, observaciones = ?, seleccionar = ?, "
                    + "id_usuario = ?, f_cambio = ? WHERE id_tipo_notificacion = ?";
            ...
            save.setString(6, tipo.getId_tipo_notificacion());
            error.setError(save.execute());
        }catch (SQLException e){
    ...
    public ErrorEntity deleteTipoNotificacion(Connection con, String id){
        ErrorEntity error = new ErrorEntity(true);
        String query = "DELETE FROM cat_tipo_notificaciones WHERE id_tipo_notificacion = '"+id+"'";
        try{
    ...
    public String getId(){
        ...
        Connection con = getConexionBD();
        ResultSet rs = null;
        String query = "SELECT IFNULL(MAX(CAST(SUBSTRING(id_tipo_notificacion,2) as SIGNED INTEGER)),0) FROM cat_tipo_notificaciones";
        try{
            Integer valor = 0;
            rs = con.createStatement().executeQuery(query);
            while (rs.next()){
    ...
    public ErrorEntity updateSeleccionar(Connection con){
        ErrorEntity error = new ErrorEntity(true);
        try{
            error.setError(con.prepareStatement("UPDATE cat_tipo_notificaciones SET seleccionar = false").execute());
        }catch (SQLException e){
            error.setTipo(new ErroresClase(e,"Error al modificar los datos").getErrorSql());
        }catch (Exception ex){
            error.setTipo(new ErroresClase(ex,"Error al modificar los datos").getErrorJava());
