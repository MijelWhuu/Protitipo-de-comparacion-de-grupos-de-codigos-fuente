...
import bl.catalogos.TipoAutosSriaAcuerdosBl;
 ...
 * @author mavg
 */
public class TipoAutosSriaAcuerdos extends javax.swing.JInternalFrame {

    SesionEntity sesion;
    TipoAutosSriaAcuerdosBl bl = new TipoAutosSriaAcuerdosBl();
     ...
     */
    public TipoAutosSriaAcuerdos(){
        initComponents();
    }
    
    public TipoAutosSriaAcuerdos(SesionEntity sesion) {
        this.sesion = sesion;
        initComponents();
        formatosCampos();
        mapeoTeclas();
    ...
    private void inicializaCombo(){
        u.inicializaCombo(cmbDestino, new TipoAutoSriaAcuerdosEntity(), bl.getTipoAutosSria());
    }
    ...
    private void botonF2(){
        if(jTabbedPane1.getSelectedIndex()==0){
            if(txtNombreNew.getText().length()==0){
                ...
                ErrorEntity error = bl.saveTipoAutoSria(tipo, sesion);
                if(!error.getError()){
    ...
    private void botonF4(){
        if(jTabbedPane1.getSelectedIndex()==1 && cmbDestino.getItemCount()>1){
            if(cmbDestino.getSelectedIndex()==0){
                ...
                TipoAutoSriaAcuerdosEntity tipo = (TipoAutoSriaAcuerdosEntity)cmbDestino.getSelectedItem();
                ...
                ErrorEntity error = bl.updateTipoAutoSria(tipo, sesion);
                if(!error.getError()){
        ...
        if(jTabbedPane1.getSelectedIndex()==1 && cmbDestino.getItemCount()>1){
            if(cmbDestino.getSelectedIndex()==0){
                ...
                if(javax.swing.JOptionPane.showInternalConfirmDialog(this, "¿Desea eliminar el elemento seleccionado?", "Alerta", 0)==0){
                    ErrorEntity error = bl.deleteTipoAutoSria(((TipoAutoSriaAcuerdosEntity)cmbDestino.getSelectedItem()).getId_tipo_auto_sria_acuerdos());
                    if(!error.getError()){
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Catálogo - Tipo de Autos de la Secretaría de Acuerdos");
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/Cat_tipo_autos_secretaria.jpg"))); // NOI18N
        ...
        jPanel2.add(jLabel5);
        jLabel5.setBounds(20, 110, 190, 30);
