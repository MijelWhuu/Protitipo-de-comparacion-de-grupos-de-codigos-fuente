...
import bl.catalogos.OficinasBl;
...
public class Oficinas extends javax.swing.JInternalFrame {
    ...
    public Oficinas(SesionEntity sesion) {
        this.sesion = sesion;
        initComponents();
        formatosCampos();
        mapeoTeclas();
    ...
    private void inicializaCombo(){
        u.inicializaCombo(cmbDestino, new OficinaEntity(), bl.getOficinas());
    }
    ...
    private void botonF2(){
        if(jTabbedPane1.getSelectedIndex()==0){
            if(txtNombreNew.getText().length()==0){
                ...
                ErrorEntity error = bl.saveOficina(new OficinaEntity(txtNombreNew.getText(),txtObservacionesNew.getText()),sesion);
                if(!error.getError()){
    ...
    private void botonF4(){
        if(jTabbedPane1.getSelectedIndex()==1 && cmbDestino.getItemCount()>1){
            if(cmbDestino.getSelectedIndex()==0){
                ...
                OficinaEntity tipo = (OficinaEntity)cmbDestino.getSelectedItem();
                tipo.setNombre(txtNombreUpdate.getText());
                tipo.setObservaciones(txtObservacionesUpdate.getText());
                ErrorEntity error = bl.updateOficina(tipo, sesion);
                if(!error.getError()){
        ...
        if(jTabbedPane1.getSelectedIndex()==1 && cmbDestino.getItemCount()>1){
            if(cmbDestino.getSelectedIndex()==0){
                ...
                if(javax.swing.JOptionPane.showInternalConfirmDialog(this, "¿Desea eliminar el elemento seleccionado?", "Alerta", 0)==0){
                    ErrorEntity error = bl.deleteOficina(((OficinaEntity)cmbDestino.getSelectedItem()).getId_oficina());
                    if(!error.getError()){
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/Cat_oficinas.jpg"))); // NOI18N
        ...
        jLabel7.setText("Nombre de la Oficina");
        jPanel1.add(jLabel7);
        jLabel7.setBounds(20, 40, 240, 30);

        txtNombreNew.setFont(new java.awt.Font("Tahoma", 0, 13)); // NOI18N
        jPanel1.add(txtNombreNew);
        txtNombreNew.setBounds(10, 70, 390, 30);
        ...
        jScrollPane3.setBounds(10, 140, 390, 100);
        ...
        cmbDestino.setBounds(80, 30, 290, 30);

        jPanel2.add(jPanel3);
        jPanel3.setBounds(10, 11, 390, 80);

        jLabel5.setFont(new java.awt.Font("Tahoma", 1, 13)); // NOI18N
        jLabel5.setText("Nombre de la Oficina");
        jPanel2.add(jLabel5);
        jLabel5.setBounds(20, 90, 320, 30);
        ...
        jScrollPane2.setBounds(10, 190, 390, 80);
